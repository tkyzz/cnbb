	...
	$.validationEngineLanguage = {
		newLang:function(){
			$.validationEngineLanguage.allRules = {
				...
				"mobile":{
					"regex":/^0?(13[0-9]|15[0-9]|18[0-9]|14[0-9]|17[0-9])[0-9]{8}$/,
					"alertText":"* 无效的 手机 号码"
				},
				"quhao":{
					"regex":/^0[0-9]{2,3}$/,
                ...
                "checkNameLength": { //全称简称 custom[checkNameLength]
                    "func": function(field, rules, i, options){
				        fullname = $('[name="com[fullname]"]').val().length;
				        abbrname = $('[name="com[abbrname]"]').val().length;
				        //alert(fullname+''+abbrname);
				        if(fullname == 0 || abbrname == 0){
				            return true;
				        }else if(fullname < abbrname){
				            return false;
				        }else{
				        	return true;
				        }
				...
				"post_xinzi": { //全称简称 custom[checkNameLength]
                    "func": function(field, rules, i, options){
				        //alert(fullname+''+abbrname);
				        return false;
				        var val = $('[name="post[ssalary]"]').val();
				        if($('[name="post[is_mianyi]"]').attr('checked') != 'checked'){
				        	//alert(val.length);
				        	if(val.length == 0){
				        		return false;
				        	}
